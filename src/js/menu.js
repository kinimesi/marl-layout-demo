...
$("body").on("click", "#runLayout", function(){
  ...
  if(layoutType.options[layoutType.selectedIndex].text == "fCoSE") {
    ...
    cy.layout({name: "fcose", padding: 20, quality: quality, randomize: randomize, packComponents: true}).run();
    endTime = performance.now();
    evaluate(endTime - startTime);
  }
  else if(layoutType.options[layoutType.selectedIndex].text == "MARL"){
    startTime = performance.now();
    cy.layout({name: "cose-bilkent", padding: 20, quality:quality, randomize: randomize, animationDuration: 1000,
  ...
  else if(layoutType.options[layoutType.selectedIndex].text == "FD"){
    startTime = performance.now();
